 ...
 * 2. 자동차이름이라는 변수가 "클래스 선언 명령문(public class Car)" 아래에 바로 선언과 초기화(자동차 문자열을 할당)가 되어 있다.
 * 3. 변수 선언문 앞에 public이라는 키워드가 부착되어 있다.
 * 4. 전진()...의 Method 코드가 있고, 이 Method 코드를 선언문이라 한다.
 * 5. Car 클래스를 만들어 사용하는 이유는 비슷한 기능을 한 번만 만들고 여러 곳에서 사용하기 위함이다.
 */
