package com.biz.grade;

import java.util.Random;

/*
 * 학생 20명, 과목 5과목의 성적 처리
 */
public class Grade_02 {
	public static void main(String[] args) {
		
		int stdCount=20; //학생 인원 수
		int subCount=5+2; //과목 5개와 총점, 평균을 저장(직관적으로 보기 위해 연산자 사용)
		
		/*
		 * 배열을 정해진 갯수만큼 선언하고
		String[] subTitle=new String[subCount];
		
		정해진 갯수만큼 값을 채워 넣는 방식
		subTitle[0]="국어";
		subTitle[1]="영어";
		subTitle[2]="수학";
		subTitle[3]="음악";
		subTitle[4]="미술";
		*/
		
		/*
		 * 배열을 선언할 때 보관할 값의 갯수가 얼마인지 확실치 않을 때 배열을 빈(blank) 크기만큼만 선언하고
		 * {};을 사용하여 값을 미리 저장하는 방법
		 * 이 방법을 사용하면 배열의 갯수가 변동될 때, 배열에 저장되는 값을 추가, 삭제함으로써 자동으로 배열이 생성됨
		 * 배열을 선언할 때 값으로 크기를 지정하는 방법
		 */
		String[] subTitle=new String[] {
				"학번","국어","영어","수학","음악","미술","총점","평균"
		};
		
		//score 배열을 지정한 학생 수만큼 생성
		//20x5개의 공간을 갖는 2차원 배열 선언
		int[][] score=new int[stdCount][subCount];
		//int[][] score : score배열을 가로세로 방향으로 선언
		//				2차원 배열, Table Array

		//score[0]=new int[subCount];
		//score[1]=new int[subCount];
		//score[2]=new int[subCount];
		
		//세로 방향으로 5번 위치, 가로 방향으로 2번 위치에 값 90을 저장
		score[5][2]=90;
		
		Random rnd=new Random();
		
		//index를 줄여서 임시로 사용하는 변수명
		for(int i=0;i<stdCount;i++) {
			for(int j=0;j<subCount;j++) {
				//0,0부터 0,4까지
				//1,0부터 1,4까지
				//		~
				//19,0부터 19,4까지 반복
				score[i][j]=rnd.nextInt(50)+51;
			}
		}
		
		System.out.println("\t\t\t성적일람표");
		System.out.println("=============================================================");
		/*
		 * 배열.length : 배열의 갯수가 몇 개인지 알려주는 변수로, 키워드 변수, 필드 변수, 멤버 변수라고 한다.
		 * 				배열의 갯수가 궁금할 때 일일이 세어보지 않아도 알 수 있게 해준다. 
		 */
		for(int i=0;i<subTitle.length;i++) {
			System.out.printf("%s\t",subTitle[i]);
		}
		System.out.println("\n-------------------------------------------------------------");
		for(int i=0;i<stdCount;i++) {
			System.out.printf("%d\t",(i+1));
			for(int j=0;j<subCount;j++) {
				System.out.printf("%d\t",score[i][j]);
			}
			System.out.println();
		}
		System.out.println("-------------------------------------------------------------");
		
	}

}
